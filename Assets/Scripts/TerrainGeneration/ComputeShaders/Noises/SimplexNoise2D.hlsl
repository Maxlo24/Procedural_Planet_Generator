#ifndef _INCLUDE_SIMPLEX_NOISE_ 
#define _INCLUDE_SIMPLEX_NOISE_
...
// BEGIN JULIEN'S MODIFICATIONS

void SimplexNoises2D_float(float2 input, int octaveCount, float2 offset, float scale, out float Out, out float OutElevation)
{
	float noise = 0.0;
	float noiseSumElevation = 0.0;
	float frequency = 1.0;
	float amplitude = 2.0;
	...
	for (int i = 0; i < octaveCount; i++)
	{
		...
		float noiseI = amplitude * snoise((input + offset) * frequency * scale);
		noise += noiseI;
		noiseSumElevation += sumElevation * noiseI;
		frequency *= 2.0;

	}

		noise = noise / sumPersistency;
		noiseSumElevation = noiseSumElevation / sumPersistency;


	Out = noise;
	OutElevation = noiseSumElevation;
}

// END JULIEN'S MODIFICATIONS
