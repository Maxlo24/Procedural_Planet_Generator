...
public class CraterGeneration : MonoBehaviour
{
    ...
    public RenderTexture GenerateCraters(RenderTexture rt, Terrain terrain)
    {
        ...
        for (int i = 0; i < Number; i++)
        {
            ...
            Vector2 vecMin = craters.Position[i] - new Vector2(radius, radius) / 2.0f;
            Vector2 vecMax = craters.Position[i] + /*new Vector2(1, 1)*/new Vector2(radius, radius) / 2.0f;
            float minCenter = ImageLib.GetMinFromRenderTexture(rt, (int)vecMin.x, (int)vecMax.x, (int)vecMin.y, (int)vecMax.y);
